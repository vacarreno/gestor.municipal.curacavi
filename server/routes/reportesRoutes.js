...
router.get("/inspeccion/:id/pdf", auth, async (req, res) => {
  ...
  try {
    ...
    const doc = new PDFDocument({
      ...
      bufferPages: true, // permite numerar páginas
    });
    doc.pipe(res);
    ...
    const baseItems = [
      ...
      "Agua del radiador","Líquido de freno","Correas","Agua de batería","Extintor","Botiquín","Gata",
      "Llave de ruedas","Triángulos","Chaleco reflectante","Limpia parabrisas","Herramientas",
      "Cinturón de seguridad","Espejos laterales","Espejo interior","Radiotransmisor","Bocina de retroceso",
      "Antena","Permiso de circulación","Revisión técnica","Seguro obligatorio","Techo","Capot","Puertas",
      "Vidrios","Tapabarros","Pick-up","Parachoques","Tubo de escape","Aseo de cabina","Sanitización COVID-19",
    ];
    ...
    if (data.rut_conductor) doc.text(`RUT: ${data.rut_conductor}`, L() + 15, baseY + 24);
    doc.text("_____________________________", L() + 260, baseY);
    doc.text("Supervisor:", L() + 270, baseY + 12);

    // === PIE DE PÁGINA (numeración) ===
    const range = doc.bufferedPageRange(); // obtiene total de páginas
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc.fontSize(8).fillColor("#666").text(footerText, x, y);
    }
